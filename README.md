...
**NOTE on Deployment scripts** :- Please try running solution and deployment scripts only in a sandbox/test env.**(Not for Production use)**. **Don't use real data for testing , please use either the test cases provided in this repo or your own mocked input templates**. We are working on hardening the solution with best practices and making azure deployment as simple as possible and will notify you once the repo is updated.
